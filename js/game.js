...
window.onload = async function() {
    ...
    document.getElementById('play-button').addEventListener('click', () => {
        ...
        // Make sure to reset game mode flags
        snakeGame.reset();
        snakeGame.isInfiniteMode = false;
        snakeGame.isParanoidMode = false;
    ...
    function startInfiniteMode() {
        ...
        snakeGame.isParanoidMode = false;
        // Disable wall collision for Infinite mode (allow wrapping)
        snakeGame.hasWallCollision = false;
        snakeGame.start();
    }
    ...
    function startParanoidMode() {
        ...
        // Set up snake with 180 blocks
        snakeGame.reset();
        
        // Ensure snake has exactly 180 blocks total
        const currentLength = snakeGame.snake.segments.length;
        if (currentLength < 180) {
            // Add segments until we reach 180
            ...
            // Remove segments until we reach 180
        ...
        // Set up automatic random movement for paranoid mode
        if (snakeGame.paranoidModeInterval) {
            clearInterval(snakeGame.paranoidModeInterval);
        }
        
        // Create interval for random direction changes
        snakeGame.paranoidModeInterval = setInterval(() => {
            if (snakeGame.isGameOver || !snakeGame.isParanoidMode) {
                clearInterval(snakeGame.paranoidModeInterval);
                return;
            }
            ...
            const currentDirection = snakeGame.snake.direction;
            
            // Filter out the opposite of the current direction to avoid reversing
            ...
            snakeGame.snake.setDirection(randomDirection);
            
            console.log(`Paranoid mode: Changed direction to ${randomDirection}`);
        }, 500); // Change direction every 500ms
